...
class MutexWatershedTest {
    ...
    @Test
    fun `prim single chain with one mutex edge`() {
        ...
        val edgeNeighbors = MutexWatershed.DefaultEdgeNeighbors(
                TLongObjectHashMap<LongArray>()
                        .also { it.put(0, longArrayOf(1, 3)) }
                        .also { it.put(1, longArrayOf(0, 2)) }
                        .also { it.put(2, longArrayOf(1, 3)) }
                        .also { it.put(3, longArrayOf(0, 2)) },
                TLongObjectHashMap<IntArray>()
                        .also { it.put(0, intArrayOf(0, 3)) }
                        .also { it.put(1, intArrayOf(0, 1)) }
                        .also { it.put(2, intArrayOf(1, 2)) }
                        .also { it.put(3, intArrayOf(2, 3)) })

        // with array store
        run {
            val mutexStorage = MutexStorageArray(4)
            val uf = MutexWatershed.computeMutexWatershedClusteringPrim(
                    4,
                    edges,
                    mutexEdges,
                    edgeWeights,
    ...
    @Test
    fun `prim simple 2D test with 3 regions`() {
        // expected labeling (ids may differ):
        // 1 - 1 - 2 - 3
        //
        // 1 - 2 - 2 - 3
        ...
        fun Collection<Pair<Pair<Int, Int>, Double>>.findNeighborsFor(node: Long) = this
                .map { it.first }
                .map { e -> if (e.first == node.toInt()) e.second else if (e.second == node.toInt()) e.first else null }
                .filterNotNull()
                .map { it.toLong() }
                .toLongArray()

        fun Collection<Pair<Pair<Int, Int>, Double>>.findEdgeIdsFor(node: Long) = this
                .map { it.first }
                .mapIndexed { index, e -> if (e.first == node.toInt() || e.second == node.toInt()) index else null }
                .filterNotNull()
                .toIntArray()

        val edgeNeighbors = MutexWatershed.DefaultEdgeNeighbors(
                TLongObjectHashMap<LongArray>()
                        .also { m -> nodes.forEach { m.put(it, nearestNeighborEdges.findNeighborsFor(it) + longDistanceEdges.findNeighborsFor(it)) } },
                TLongObjectHashMap<IntArray>()
                        .also { m -> nodes.forEach { m.put(it, nearestNeighborEdges.findEdgeIdsFor(it) + longDistanceEdges.findEdgeIdsFor(it)) } })
